...
datax<-select(data,crowflies_address_infstlmnt,Cnt_IS_see,Cnt_IS_smell,Cnt_IS_hear,
  ...
  xval<-matrix(nrow=200,ncol=8)
  colnames(xval)<-c("rmsea_1","cfi_1","tfi_1","srmr_1","rmsea_2","cfi_2","tfi_2","srmr_2",)
...
# Now we need to run, and gather some of the fit indices 
  ...
  x<-fitmeasures(medcloseness2fit,c("rmsea","cfi","tfi","srmr"))
  xval[i,1]<-x[1]
  xval[i,2]<-x[2]
  xval[i,3]<-x[3]
  xval[i,4]<-x[4]
  ...
  y<-fitmeasures(medcloseness2fit,c("rmsea","cfi","tfi","srmr"))
  xval[i,5]<-y[1]
  xval[i,6]<-y[2]
  xval[i,7]<-y[3]
  xval[i,8]<-y[4]
